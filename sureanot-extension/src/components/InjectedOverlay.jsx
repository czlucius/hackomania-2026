        ...
        const sendAnalysis = () => {
            chrome.runtime.sendMessage({ type: 'ANALYZE_MESSAGE', text }, (response) => {
                if (chrome.runtime.lastError) {
                    console.warn(`SureAnot.ai: Message channel closed (attempt ${retries + 1}):`, chrome.runtime.lastError.message);
                    if (retries < maxRetries) {
                        retries++;
                        setTimeout(sendAnalysis, 2000 * retries); // exponential backoff
                    } else {
                        console.error('SureAnot.ai: Max retries reached, giving up.');
                        setAssessment({
                            trust_score: null,
    ...
    if (loading) {
        return (
            <div className="w-full pointer-events-auto mt-1 flex px-3 py-2 rounded-lg border bg-gray-50 text-gray-500 shadow-sm items-center gap-2">
                ...
                <span className="text-[13px] font-medium font-sans">Analyzing with SureAnot.ai...</span>
            </div>
        );
    }
